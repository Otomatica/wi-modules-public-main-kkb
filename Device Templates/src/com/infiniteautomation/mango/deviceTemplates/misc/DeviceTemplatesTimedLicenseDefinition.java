...
public class DeviceTemplatesTimedLicenseDefinition implements ITimedLicenseDefinition {
    private boolean licensed;

    public DeviceTemplatesTimedLicenseDefinition() {
        this.licensed = false;
    }

    public void addLicenseErrors(final List<TranslatableMessage> errors) {
        if (!this.licensed) {
            errors.add(new TranslatableMessage("deviceTemplates.event.freeModeSimple"));
        }
    }
    ...
    public long licenseCheck() {
        this.licensed = WiseIfLicenseChecker.checkLicense("deviceTemplates");
        return this.licensed ? -1L : Common.getMillis(3, 2);
    }

    public String getShutdownDescriptionKey() {
        return "deviceTemplates.event.freeMode";
    }
}
